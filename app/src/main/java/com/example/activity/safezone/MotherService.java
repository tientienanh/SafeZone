                        ...
                        public void queryRouteSuccess(List<ParseObject> parseObjects) {
                                ...
                                for (int j = 0; j < routes.size(); j++) {
                                    for (int i = 0;i<parseObjects.size();i++) {
                                        String childName = parseObjects.get(i).getString(LoginSonActivity.CHILD_NAME);
                                        if (childName.equals(routes.get(j).getChildrenName())) {
                                            lat = parseObjects.get(i).getDouble(LoginSonActivity.CHILD_LATITUDE);
                                            llong = parseObjects.get(i).getDouble(LoginSonActivity.CHILD_LONGITUDE);
